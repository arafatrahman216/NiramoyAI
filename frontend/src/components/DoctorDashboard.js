...
const DoctorDashboard = () => {
  ...
  const fetchDoctorData = async () => {
    try {
      ...
      const profileResponse = await axios.get('http://localhost:8080/api/doctors/profile');
      setDoctorProfile(profileResponse.data.doctor);

      // Fetch dashboard stats
      const statsResponse = await axios.get('http://localhost:8080/api/doctors/dashboard/stats');
      setStats(statsResponse.data.stats);

      // Fetch today's appointments
      const today = new Date().toISOString().split('T')[0];
      const appointmentsResponse = await axios.get(`http://localhost:8080/api/doctors/appointments?startDate=${today}&endDate=${today}`);
